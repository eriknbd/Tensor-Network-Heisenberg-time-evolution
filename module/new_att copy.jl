...
function step(ψ::MPS, gate; cutoff=1e-10, maxdim=nothing, hermitianize::Bool = false)
    ψ = apply(gate, ψ; cutoff=cutoff, maxdim=maxdim)
    _normalize!(ψ)
    return ψ
end

function step(ρ::MPO, gate; cutoff=1e-10, maxdim=nothing, hermitianize::Bool = false)
    ρ = apply(gate, ρ; cutoff=cutoff, maxdim=maxdim, apply_dag=true)
    if hermitianize
        hermitianize!(ρ, normalize = true)
    else
        _normalize!(ρ)
    end
    return ρ
...
function state_evo(s, psi, t_Vec, r; h::Vector = [nothing], per:: Bool = false, cutoff::Real = 1e-10, 
                    showprogress::Bool = true, hermitianize::Bool = false)
    ...
    for (i, _) in enumerate(t_Vec)
        ...
        psi_t = step(psi_t, gate; cutoff=cutoff, hermitianize = hermitianize)
...
### Cálculo de parámetros de cada estado
...
function Diff_trotter_r(s, psi,  t,  rv::Vector; h::Vector = [nothing], per:: Bool = false,  
                        cutoff::Real = 1e-10, doprint::Bool = true, showprogress::Bool = false, hermitianize::Bool = false)
    N = length(s) 
    ...
    for r in rv
        ...
        evo = state_evo(s, psi, ts, r; h, per = per, showprogress = showprogress, hermitianize = hermitianize)
        push!(All_Full_evos, state_evo_parameters(s, evo, showprogress = showprogress))
    end
...
function plot_diff(All_evos, t; plot_diffs::Bool = true)
    ...
    for i in 1:(L-1)
        dmps = states_distance(MPSv_all[i], MPSv_all[L][idv[i]])
        push!(dist_MPS, dmps)
